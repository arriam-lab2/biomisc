...
from typing import TypeVar, Generator, List, Tuple, \
    Type, Generic, SupportsFloat, SupportsInt, Callable, Any, Union
from core import Map, Router, pcompile, AmbiguousError, NoRouteError
...
def _transform_str(arg: str) -> str:
    string_size = 10
    postfix = ''.join(random.choices(string.ascii_letters + string.digits, k=string_size))
...
def _get_map_function(domain: Type[D_co], codomain: Type[C_co]) -> Callable[[D_co], C_co]:
    dispatched = _map_f.dispatch(codomain, domain)
    if not dispatched:
        raise RuntimeError(f"Could not find a mapping from {domain} to {codomain}")
    return F(dispatched, None)


def _get_map(domain: Type[D_co], codomain: Type[C_co]) -> Map[D_co, C_co]:
...
def _generate_graph(nodes: List[TypeNode], count: int) -> SortedGraph:
    return [random.sample(copy.deepcopy(nodes), random.randint(1, len(nodes))) for _ in range(count)]


def _generate_routers(domains: List[Type], num_layers: int) -> Tuple[List[Router], SortedGraph]:
    generated = False
    ...
    while not generated:
        ...
        routers = [Router('', list(reduce(itertools.chain, [n.edges for n in layers[i]])))
...
def _generate_pcompile_test(domains: List[Type],
                            num_layers: int,
                            num_tests) -> List[Tuple[List[Router], SortedGraph, Type[A], Type[B]]]:
    assert num_layers > 0
    routers, layers = _generate_routers(domains, num_layers)
    return [(routers, layers, random.choice(routers[0].domains), random.choice(routers[-1].codomains))
...
@given(sampled_from(_generate_pcompile_test([Int, Float, Str, int, str, float, Foo], 3, 50)))
def test_pcompile(test_data: Tuple[List[Router], SortedGraph, Type[A], Type[B]]):
    routers, layers, input, output = test_data
    assert (len(routers) > 0) and (len(routers) == len(layers))
...
if __name__ == "__main__":
    routers, layers, input, output = random.choice(
        _generate_pcompile_test([Int, Float, Str, int, str, float, Foo], 3, 10)
    )

    input_node = list(filter(lambda n: n.value == input, layers[0]))[0]
    output_node = list(filter(lambda n: n.value == output, layers[-1]))[0]
    #print(routers, layers, input_node, output_node)
    #print(_bfs(layers, input_node, output_node))

    #compiled = pcompile(routers, input, output)
    #print(compiled)
